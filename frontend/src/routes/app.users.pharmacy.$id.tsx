...
import { ArrowLeft, Mail, Phone, MapPin, Building2, ExternalLink, Package, Truck, Boxes, ShieldAlert, Copy, Loader2 } from "lucide-react";
...
export const Route = createFileRoute("/app/users/pharmacy/$id")({
...
function PharmacyDetail() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await api.get(`/partners/pharmacy/${id}`);
        const d = res.data;
        setPharmacy({
          id: d.id,
          name: d.name,
        ...
        toast.error(error.response?.data?.error || "Failed to load pharmacy");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!pharmacy) {
    return <div className="p-6 text-center text-sm text-muted-foreground">Pharmacy not found.</div>;
  }
  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto">
      ...
      <PageHeader title={pharmacy.name} description={pharmacy.description}>
      ...
      <div className="flex flex-wrap gap-4 mb-6 text-xs text-muted-foreground bg-surface rounded-xl p-4 border border-border">
        <span className="flex items-center gap-1.5">
          <Building2 className="h-3 w-3" /> {pharmacy.region}
        </span>
        <span className="flex items-center gap-1.5">
          <Mail className="h-3 w-3" /> {pharmacy.email}
        </span>
        <span className="flex items-center gap-1.5">
        ...
        <span>Joined {new Date(pharmacy.joinedDate).toLocaleDateString()}</span>
      </div>
      ...
      <SectionPanel title="Related Records" description="View detailed records for this pharmacy">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <Link to="/app/orders" search={{ q: pharmacy.id }}>
            <Button variant="outline" className="w-full justify-start gap-3 h-16 group hover:border-primary/50 transition-colors">
              ...
              <div className="flex flex-col items-start flex-1 min-w-0">
                <span className="font-medium text-sm group-hover:text-primary transition-colors">View Orders</span>
                <span className="text-[10px] text-muted-foreground truncate w-full text-left">Filter orders by this pharmacy</span>
              </div>
              <ExternalLink className="h-4 w-4 text-muted-foreground shrink-0" />
            </Button>
          </Link>
          
          <Link to="/app/shipments" search={{ q: pharmacy.id }}>
            <Button variant="outline" className="w-full justify-start gap-3 h-16 group hover:border-primary/50 transition-colors">
              <div className="h-10 w-10 rounded-lg bg-primary/10 grid place-items-center shrink-0">
              ...
              <div className="flex flex-col items-start flex-1 min-w-0">
                ...
                <span className="text-[10px] text-muted-foreground truncate w-full text-left">Filter shipments by this pharmacy</span>
              </div>
              <ExternalLink className="h-4 w-4 text-muted-foreground shrink-0" />
            </Button>
          </Link>

          <Link to="/app/medicines" search={{ q: pharmacy.id }}>
            <Button variant="outline" className="w-full justify-start gap-3 h-16 group hover:border-primary/50 transition-colors">
              <div className="h-10 w-10 rounded-lg bg-primary/10 grid place-items-center shrink-0">
              ...
              <div className="flex flex-col items-start flex-1 min-w-0">
                ...
                <span className="text-[10px] text-muted-foreground truncate w-full text-left">Filter medicines by this pharmacy</span>
              </div>
              <ExternalLink className="h-4 w-4 text-muted-foreground shrink-0" />
            </Button>
          </Link>
