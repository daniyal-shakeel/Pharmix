...
import { ArrowLeft, Truck, Star, MapPin, Mail, Phone, ExternalLink, ShieldAlert, Copy, Loader2 } from "lucide-react";
...
export const Route = createFileRoute("/app/users/delivery/$id")({
...
function DeliveryDetail() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await api.get(`/partners/delivery/${id}`);
        const d = res.data;
        setPartner({
          id: d.id,
          name: d.name,
          ...
          rating: d.rating || 0,
          totalDeliveries: d.skus || 0,
          linkedManufacturers: [],
        });
      } catch (error: any) {
        toast.error(error.response?.data?.error || "Failed to load delivery partner");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto">
      ...
      <PageHeader title={partner.name} description={`Delivery partner · ${partner.zone}`}>
      ...
      <div className="flex flex-wrap gap-4 mb-6 text-xs text-muted-foreground bg-surface rounded-xl p-4 border border-border">
        ...
        <span className="flex items-center gap-1.5">
          <Truck className="h-3 w-3" /> {partner.vehicle}
        </span>
        <span className="flex items-center gap-1.5">
          <MapPin className="h-3 w-3" /> {partner.zone}
        </span>
        <span>Joined {new Date(partner.joinedDate).toLocaleDateString()}</span>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <StatCard label="Rating" value={partner.rating.toString()} icon={Star} />
      </div>
      ...
      <SectionPanel title="Related Records" description="View detailed records for this delivery partner">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <Link to="/app/shipments" search={{ q: partner.id }}>
            <Button variant="outline" className="w-full justify-start gap-3 h-16 group hover:border-primary/50 transition-colors">
              ...
              <div className="flex flex-col items-start flex-1 min-w-0">
                ...
                <span className="text-[10px] text-muted-foreground truncate w-full text-left">Filter shipments by this partner</span>
              </div>
